...
| `mask_mode` | строка | Режим маскирования: `"mask"` (звёздочки) или `"tokenize"` (токены). По умолчанию `"mask"`. |
| `mask_combinations` | массив | Контекстное маскирование: список комбинаций типов ПД, при которых маскировать. Например, `[["PIN", "CARD"]]` — маскировать только если найдены и PIN, и CARD. Пусто — маскировать всегда. |
